...
Intermediate microeconomics discusses two market failures that (plausibly) require state intervention: non-excludable goods and externalities. The third one is **credit restrictions**
...
An **actuarially fair insurance premium** is a insurance plan in which its price is exactly equal to the insurer’s expected payout

In practice, the insurer may charge a **risk premium**, which is the difference between the expected payout and the *certainty equivalent*
...
There are two types of relevant informational asymmetry in the insurance market (any market really): **unobservable characteristics** (adverse selection) and **unobservable actions** (moral hazard): let us start with the first one
...
Also, if I do not have car insurance and crash into someone, I may not have the money to cover their damage, generating a *financial externality* (that is why DPVAT – Seguro Obrigatório para Proteção de Vítimas de Acidentes de Trânsito – exits)

Another reason for public insurance is that governments may not be able to *credibly commit* to not intervene in the bad state of nature in case the agent does not buy insurance (**Samaritan's dilemma**)
...
Another essential concept of asymmetric information in the insurance market is the effect that insurance has on consumers' risk taking (**moral hazard**)
...
Consider an individual who has probability $p$ to be unemployed, that has consumption $c_e = w - t$ when employed (where $w$ is the salary and $t$ is the tax) and $c_u = b$ when unemployed, with $b$ as benefit
...
Government has a **balanced budget**: $(1 - p) t = p b$, or $t = b p/(1-p)$
...
There is a **fiscal externality**: the higher the unemployment insurance, the higher the taxation we will need to finance it, both *mechanically*, but also due to the *behavioral effect*
...
Every decade, the problem of the sustainability of Brazilian retiremet system and reforms to decrease social security payments come back to the public debate. [The last one was in 2019](https://www.gov.br/inss/pt-br/assuntos/noticias/confira-as-principais-mudancas-da-nova-previdencia):
...
The value is 80% (up to $2,041, 50% thereafter) of the average wage of the last 3 months of work (**replacement rate**), with a maximum of $2,313 (2024), and the worker is paid for 3 (up to 1 year), 4 (up to 2) or 5 months (more than 2 years of work)

Unemployment benefit can never be below the minimum wage in Brazil ($1,412 in 2024), however
